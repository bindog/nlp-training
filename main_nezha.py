...
def get_model(task_name, bert_config, num_labels):
    if task_name == "ner":
        return BertForTokenClassification(bert_config, num_labels=num_labels)
    elif task_name == "text-clf":
...
def convert_examples_to_features_ner(examples, label_list, max_seq_length, tokenizer, label_map_training=None,
                                     my_tokenization=False):
    """Loads a data file into a list of `InputBatch`s."""
...
def get_dataloader(args, processor, label_list, tokenizer, split):
    if split == "train":
    ...
    features = convert_examples_to_features_ner(
        examples, label_list, args.max_seq_length, tokenizer, my_tokenization=args.my_tokenization)
    ...
    if split == "train":
        if args.local_rank == -1:
            ...
            data_sampler = DistributedSampler(tensor_data)
        dataloader = DataLoader(tensor_data, sampler=data_sampler, batch_size=args.train_batch_size)
        return dataloader
    elif split == "dev" or split == "val" or split == "test":
        data_sampler = SequentialSampler(tensor_data)
        dataloader = DataLoader(tensor_data, sampler=data_sampler, batch_size=args.eval_batch_size)
        return dataloader
    else:
        return None


def train_ner_loop(args, model, device, train_dataloader, optimizer, n_gpu, global_step):
    model.train()
    tr_loss = 0
...
def ner_eval(args, model, device, eval_dataloader, label_map_reverse):
    '''
        label_map_reverse: from id to tag, like 5 -> ORG-I
    '''
    model.eval()
...
def text_clf_eval(args, model, device, eval_dataloader):
    eval_all_result = []
...
def main():
    ...
    num_labels_task = {
        ...
        "xnli": 3,
        "text-clf": 0,
        "ner": 11  # token classification + 1 (for zero padding length)
    }
    ...
    if args.task_name == "ner":
        # TODO should consistent with ner_features settings
        label_map = {label : i for i, label in enumerate(label_list,1)}
        label_map_reverse = {v: k for k, v in label_map.items()}
        # FIXME temp fix
        label_map_reverse[0] = "O"
    ...
    if args.trained_model_dir:
        ...
        model = get_model(args.task_name, config, num_labels=num_labels)
        ...
        model = get_model(args.task_name, bert_config, num_labels=num_labels)
    ...
    if args.do_train:
        ...
        train_dataloader = get_dataloader(args, processor, label_list, tokenizer, "train")
        num_epoch = 0
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            train_ner_loop(args, model, device, train_dataloader, optimizer, n_gpu, global_step)
            num_epoch += 1

            # begin to evaluate
            logger.info("================ running evaluation on dev set ===================")
            if args.task_name == "ner":
                eval_dataloader = get_dataloader(args, processor, label_list, tokenizer, "dev")
                ner_eval(args, model, device, eval_dataloader, label_map_reverse)
            elif args.task_name == "text-clf":
                eval_dataloader = get_dataloader(args, processor, label_list, tokenizer, "dev")
                text_clf_eval(args, model, device, eval_dataloader)
    ...
    if args.do_test:
        ...
        if args.task_name == "ner":
            eval_dataloader = get_dataloader(args, processor, label_list, tokenizer, "test")
            ner_eval(args, model, device, eval_dataloader, label_map_reverse)
        elif args.task_name == "text-clf":
            eval_dataloader = get_dataloader(args, processor, label_list, tokenizer, "test")
            text_clf_eval(args, model, device, eval_dataloader)
